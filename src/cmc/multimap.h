...
#define CMC_GENERATE_MULTIMAP_HEADER(PFX, SNAME, K, V)                                       \
    ...
    size_t PFX##_key_count(SNAME *_map_, K key);                                             \
...
#define CMC_GENERATE_MULTIMAP_SOURCE(PFX, SNAME, K, V)                                          \
    ...
    {                                                                                           \
        ...
        if (!_map_)                                                                             \
            return NULL;                                                                        \
                                                                                                \
        _map_->buffer = calloc(real_capacity, sizeof(SNAME##_entry_ptr[2]));                    \
                                                                                                \
        if (!_map_->buffer)                                                                     \
        {                                                                                       \
            free(_map_);                                                                        \
    ...
    {                                                                                           \
        ...
        if (theoretical_size < PFX##_count(_map_) / PFX##_load(_map_))                          \
            return false;                                                                       \
                                                                                                \
        SNAME *_new_map_ = PFX##_new(capacity, PFX##_load(_map_), _map_->cmp, _map_->hash);     \
                                                                                                \
        if (!_new_map_)                                                                         \
            return false;                                                                       \
                                                                                                \
