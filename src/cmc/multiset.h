...
#define CMC_GENERATE_MULTISET_SOURCE(PFX, SNAME, V)                            \
    ...
    {                                                                          \
        struct SNAME *result =                                                 \
            PFX##_new(_set_->capacity, _set_->load, _set_->cmp, _set_->hash);  \
                                                                               \
        if (!result)                                                           \
            return NULL;                                                       \
                                                                               \
    ...
    {                                                                          \
        ...
        if (!ignore_multiplicity &&                                            \
            PFX##_cardinality(_set1_) != PFX##_cardinality(_set2_))            \
            return false;                                                      \
                                                                               \
        ...
        {                                                                      \
            ...
            if (entry == NULL)                                                 \
                return false;                                                  \
                                                                               \
            if (!ignore_multiplicity &&                                        \
                entry->multiplicity != PFX##_iter_multiplicity(&iter))         \
                return false;                                                  \
        }                                                                      \
                                                                               \
        return true;                                                           \
