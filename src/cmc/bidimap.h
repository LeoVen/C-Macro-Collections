...
#define CMC_GENERATE_BIDIMAP_HEADER(PFX, SNAME, K, V)                                   \
    ...
    {                                                                                   \
        ...
        struct SNAME##_entry_s **key_buffer;                                            \
                                                                                        \
        /* Array of entries mapping V -> K */                                           \
        struct SNAME##_entry_s **val_buffer;                                            \
                                                                                        \
    ...
    SNAME *PFX##_new(size_t capacity, double load,                                      \
    ...
    bool PFX##_update_key(SNAME *_map_, K key, K *old_key);                             \
    bool PFX##_update_val(SNAME *_map_, V val, V *old_val);                             \
    bool PFX##_remove_by_key(SNAME *_map_, K key, V *out_value);                        \
    bool PFX##_remove_by_val(SNAME *_map_, V value, K *out_key);                        \
    /* Element Access */                                                                \
    K PFX##_get_key(SNAME *_map_, V val);                                               \
    V PFX##_get_val(SNAME *_map_, K key);                                               \
    /* Collection State */                                                              \
    bool PFX##_contains_key(SNAME *_map_, K key);                                       \
    bool PFX##_contains_val(SNAME *_map_, V val);                                       \
    bool PFX##_empty(SNAME *_map_);                                                     \
    bool PFX##_full(SNAME *_map_);                                                      \
...
#define CMC_GENERATE_BIDIMAP_SOURCE(PFX, SNAME, K, V)                                           \
    ...
    static SNAME##_entry **PFX##_impl_add_entry_to_key(SNAME *_map_, SNAME##_entry *entry);     \
    static SNAME##_entry **PFX##_impl_add_entry_to_val(SNAME *_map_, SNAME##_entry *entry);     \
    ...
    {                                                                                           \
        ...
        if (!_map_)                                                                             \
            return NULL;                                                                        \
                                                                                                \
        _map_->key_buffer = calloc(real_capacity, sizeof(SNAME##_entry *));                     \
        _map_->val_buffer = calloc(real_capacity, sizeof(SNAME##_entry *));                     \
                                                                                                \
    ...
    {                                                                                           \
        ...
        {                                                                                       \
            SNAME##_entry *entry = _map_->key_buffer[i];                                        \
                                                                                                \
    ...
    {                                                                                           \
        ...
        SNAME##_entry *entry = PFX##_impl_new_entry(key, value);                                \
                                                                                                \
        SNAME##_entry **key_entry = PFX##_impl_add_entry_to_key(_map_, entry);                  \
        SNAME##_entry **val_entry = PFX##_impl_add_entry_to_val(_map_, entry);                  \
                                                                                                \
    ...
    K PFX##_get_key(SNAME *_map_, V val)                                                        \
    {                                                                                           \
        SNAME##_entry **entry = PFX##_impl_get_entry_by_val(_map_, val);                        \
                                                                                                \
        if (!entry)                                                                             \
            return (V){0};                                                                      \
                                                                                                \
    ...
    V PFX##_get_val(SNAME *_map_, K key)                                                        \
    {                                                                                           \
        SNAME##_entry **entry = PFX##_impl_get_entry_by_key(_map_, key);                        \
                                                                                                \
        if (!entry)                                                                             \
            return (V){0};                                                                      \
                                                                                                \
    ...
    {                                                                                           \
        ...
        {                                                                                       \
            ...
            {                                                                                   \
                SNAME##_entry *tmp = target->key_buffer[i];                                     \
                                                                                                \
            ...
            {                                                                                   \
                SNAME##_entry *tmp = target->key_buffer[i - 1];                                 \
                                                                                                \
                if (tmp != NULL && tmp != CMC_ENTRY_DELETED)                                    \
                {                                                                               \
                    iter->last = i - 1;                                                         \
    ...
    {                                                                                           \
        ...
        iter->start = PFX##_empty(iter->target);                                                \
                                                                                                \
        SNAME##_entry *scan = iter->target->key_buffer[iter->cursor];                           \
                                                                                                \
        iter->index++;                                                                          \
                                                                                                \
        while (1)                                                                               \
    ...
    {                                                                                           \
        ...
        size_t pos = hash % _map_->capacity;                                                    \
                                                                                                \
        SNAME##_entry *target = _map_->key_buffer[pos];                                         \
                                                                                                \
        while (target != NULL)                                                                  \
        {                                                                                       \
    ...
    {                                                                                           \
        ...
        size_t pos = hash % _map_->capacity;                                                    \
                                                                                                \
        SNAME##_entry *target = _map_->val_buffer[pos];                                         \
                                                                                                \
        while (target != NULL)                                                                  \
        {                                                                                       \
    ...
    static SNAME##_entry **PFX##_impl_add_entry_to_key(SNAME *_map_, SNAME##_entry *entry)      \
    {                                                                                           \
        SNAME##_entry **to_return = NULL;                                                       \
                                                                                                \
        size_t hash = _map_->key_hash(entry->key);                                              \
        size_t original_pos = hash % _map_->capacity;                                           \
        size_t pos = original_pos;                                                              \
                                                                                                \
        SNAME##_entry **scan = &(_map_->key_buffer[hash % _map_->capacity]);                    \
                                                                                                \
        if (*scan == NULL)                                                                      \
        {                                                                                       \
            *scan = entry;                                                                      \
    ...
    {                                                                                           \
        ...
        size_t pos = original_pos;                                                              \
                                                                                                \
        SNAME##_entry **scan = &(_map_->val_buffer[hash % _map_->capacity]);                    \
                                                                                                \
        if (*scan == NULL)                                                                      \
        {                                                                                       \
            *scan = entry;                                                                      \
