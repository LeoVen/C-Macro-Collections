...
\
        int n = snprintf(str.s, cmc_string_len, cmc_cmc_string_fmt_treemap, #SNAME, #K, #V, m_, m_->root, m_->count, \
                         m_->flag, m_->f_key, m_->f_val, m_->alloc, m_->callbacks); \
\
...
\
            if (!_map_->f_key->str(fptr, root->key) && !_map_->f_val->str(fptr, root->value)) \
                return false; \
\
            left_done = true; \
\
