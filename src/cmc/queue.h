 ...
 */
#define CMC_CMC_QUEUE_CORE(PARAMS) \
...
\
        int n = snprintf(str.s, cmc_string_len, cmc_cmc_string_fmt_queue, #SNAME, #V, q_, q_->buffer, q_->capacity, \
                         q_->count, q_->front, q_->back, q_->flag, q_->f_val, q_->alloc, q_->callbacks); \
\
