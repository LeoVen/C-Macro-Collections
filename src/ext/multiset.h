...
#define MULTISET_GENERATE(PFX, SNAME, FMOD, V)    \
    MULTISET_GENERATE_STRUCT(PFX, SNAME, FMOD, V) \
    MULTISET_GENERATE_HEADER(PFX, SNAME, FMOD, V) \
    MULTISET_GENERATE_SOURCE(PFX, SNAME, FMOD, V)

/* PRIVATE *******************************************************************/
#define MULTISET_GENERATE_HEADER_PRIVATE(PFX, SNAME, FMOD, K, V) \
    MULTISET_GENERATE_HEADER(PFX, SNAME, FMOD, V)
#define MULTISET_GENERATE_SOURCE_PRIVATE(PFX, SNAME, FMOD, K, V) \
    MULTISET_GENERATE_STRUCT(PFX, SNAME, FMOD, V)                \
    MULTISET_GENERATE_SOURCE(PFX, SNAME, FMOD, V)                \
/* PUBLIC ********************************************************************/
#define MULTISET_GENERATE_HEADER_PUBLIC(PFX, SNAME, FMOD, K, V) \
    MULTISET_GENERATE_STRUCT(PFX, SNAME, FMOD, V)               \
    MULTISET_GENERATE_HEADER(PFX, SNAME, FMOD, V)
#define MULTISET_GENERATE_SOURCE_PUBLIC(PFX, SNAME, FMOD, K, V) \
    MULTISET_GENERATE_SOURCE(PFX, SNAME, FMOD, V)               \
/* STRUCT ********************************************************************/
#define MULTISET_GENERATE_STRUCT(PFX, SNAME, FMOD, V) \
                                                      \
    struct SNAME##_s                                  \
    {                                                 \
...
#define MULTISET_GENERATE_HEADER(PFX, SNAME, FMOD, V)                                             \
    ...
    FMOD bool PFX##_iter_next(SNAME##_iter *iter, V *value, size_t *multiplicity, size_t *index); \
    FMOD bool PFX##_iter_prev(SNAME##_iter *iter, V *value, size_t *multiplicity, size_t *index); \
                                                                                                  \
/* SOURCE ********************************************************************/
#define MULTISET_GENERATE_SOURCE(PFX, SNAME, FMOD, V)                                            \
    ...
    {                                                                                            \
        ...
        {                                                                                        \
            PFX##_iter_next(&iter1, &value, &mul, &index);                                       \
            PFX##_insert(_set_r_, value);                                                        \
        }                                                                                        \
                                                                                                 \
        for (PFX##_iter_tostart(&iter2); !PFX##_iter_end(&iter2);)                               \
        {                                                                                        \
            PFX##_iter_next(&iter2, &value, &mul, &index);                                       \
            PFX##_insert(_set_r_, value);                                                        \
        }                                                                                        \
                                                                                                 \
        return _set_r_;                                                                          \
    ...
    {                                                                                            \
        ...
        {                                                                                        \
            PFX##_iter_next(&iter, &value, &mul, &index);                                        \
    ...
    {                                                                                            \
        ...
        *multiplicity = iter->cursor->multiplicity;                                              \
        *index = iter->index;                                                                    \
        iter->start = false;                                                                     \
                                                                                                 \
